...
import uuid, cgitb, cgi, sys, os,pymysql
...
def Habilitado (usuario):
    # verifica que el usuario este habilitado y no tenga vencido el plazo de acceso
    q = f'select users_enable, users_due from users where users_nom = "{usuario}"'
    cursor.execute(q)
    r = cursor.fetchone()
    #print(r)
    if r == None: return False
        ...
        if r[1] != None:
            if r[1] < datetime.now().date(): 
                print ('<h4>FECHA VENCIDA</h4>')
                return False 
        if r[0] == 1: return True
        else: 
            print ('<h4>USUARIO DESHABILITADO</h4>')
            return False
...
def Acceso (usuario, clave):
    ...
    if r == None: return False
        ...
        if clave == decrip(r[0]): return True
        else: return False
...
def AccesoSes (usuario, clave):
    ...
    if r == None: return False
        ...
        if str(clave) == str(r[0]): return True
        else: return False

def Jugador (usuario):
    #devuelve verdadero si el usuario es categoria Jugador
    q = f"select users_cat from users where users_nom = \'{usuario}\'"
    cursor.execute(q)
    r = cursor.fetchone()
    if r == None: return False
    else:
        if r[0]=="P": return True
        else: return False
